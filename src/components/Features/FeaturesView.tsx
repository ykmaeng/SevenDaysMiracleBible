...
export function FeaturesView({ onClose }: FeaturesViewProps) {
  ...
  const { enabledFeatures, toggleFeature, getOrderedFeatures } = useFeatureStore();
  const [expandedId, setExpandedId] = useState<string | null>(null);

  const orderedFeatures = getOrderedFeatures();
  const enabled = orderedFeatures.filter((f) => enabledFeatures.includes(f.id));
  const disabled = orderedFeatures.filter((f) => !enabledFeatures.includes(f.id));
